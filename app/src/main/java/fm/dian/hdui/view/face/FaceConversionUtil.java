...
public class FaceConversionUtil {
     ...
     * @return
     */
    public SpannableString getExpressionString(Context context,TextView tv, String str) {
        ...
        try {
            dealExpression(context,tv, spannableString, sinaPatten, 0);
        } catch (Exception e) {
     ...
     * @throws Exception
     */
    private void dealExpression(final Context context, final TextView tv,
                                final SpannableString spannableString, final Pattern patten, int start)
            throws Exception {
        int dipHeight = (int) context.getResources().getDimension(R.dimen.face_height);
        int emojiWidth = SysTool.convertDipToPx(context, dipHeight);
...
//            Log.i("emoji =",resId+"");
            if (resId != 0) {
                Picasso.with(context).load(resId).resize(emojiWidth, emojiWidth).into(new Target() {
                    @Override
                    public void onBitmapLoaded(Bitmap bitmap, Picasso.LoadedFrom from) {
                        ...
                        if (end < spannableString.length()) {
                            // 如果整个字符串还未验证完，则继续。。
                            try {
                                dealExpression(context,tv, spannableString, patten, end);
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        }
    ...
    private void ParseData(Context context) {
        ...
        data.addAll(0, emojiFile);

        //添加红点表情
        List<String> hdFile = FileUtils.getHDFile(context);
        data.addAll(0, hdFile);
